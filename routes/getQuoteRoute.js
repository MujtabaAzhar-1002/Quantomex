...
router.post("/submit", async (req, res) => {
  try {
    ...
    const userEmailData = {
      ...
      TextPart:
        "Thank you for submitting your quotation. We have received your request and will get back to you shortly.",
      HTMLPart: `
...
Contact Number: ${contactNumber}`,
    ...
    const userRequest = mailjet.post("send", { version: "v3.1" }).request({
      Messages: [
        {
          From: {
    ...
    res
      .status(500)
      .json({ message: "Error saving form entry", error: error.message });
  }
});
...
router.post("/deleteQuote/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error deleting quote submission",
        error: error.message,
      });
  }
});
